...
def train_collate(batch):
    imgs, targets, masks = [], [], []
    pos_masks, qua_masks, sin_masks, cos_masks, wid_masks = [], [], [], [], []
    ...
    for sample in valid_batch:
        ...
        pos_masks.append(torch.tensor(sample[3], dtype=torch.float32))
        qua_masks.append(torch.tensor(sample[4], dtype=torch.float32))
        sin_masks.append(torch.tensor(sample[5], dtype=torch.float32))
        cos_masks.append(torch.tensor(sample[6], dtype=torch.float32))
        wid_masks.append(torch.tensor(sample[7], dtype=torch.float32))

    return torch.stack(imgs, 0), targets, masks, pos_masks, qua_masks, sin_masks, cos_masks, wid_masks


def train_osgd_collate(batch):
    imgs, bboxes, labels = [], [], []
    pos_masks, qua_masks, sin_masks, cos_masks, wid_masks = [], [], [], [], []
    act_masks = []
    loss_masks = []
    ...
    for sample in valid_batch:
        imgs.append(torch.tensor(sample[0], dtype=torch.float32))
        bboxes.append(torch.tensor(sample[1], dtype=torch.float32))
        ...
        cos_masks.append(torch.tensor(sample[6], dtype=torch.float32))
        wid_masks.append(torch.tensor(sample[7], dtype=torch.float32))
        act_masks.append(torch.tensor(sample[8], dtype=torch.float32))
        loss_masks.append(torch.tensor(sample[9], dtype=torch.float32))

    return torch.stack(imgs, 0), bboxes, pos_masks, qua_masks, sin_masks, cos_masks, wid_masks, act_masks, loss_masks


def train_grasp_collate(batch):
    imgs, targets, masks,  = [], [], []
    valid_batch = [aa for aa in batch if aa[0] is not None]
