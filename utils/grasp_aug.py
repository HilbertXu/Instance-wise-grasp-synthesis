...
class Compose(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        for t in self.transforms:
            img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks = t(img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks)
        return img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
class ConvertFromInts(object):
    def __call__(self, image, masks=None, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        return image.astype(np.float32), masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
class ToAbsoluteCoords(object):
    def __call__(self, image, masks=None, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        return image, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
class Pad(object):
    ...
    def __call__(self, image, masks, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        im_h, im_w, im_c = image.shape

        expand_image = np.zeros(
            (self.height, self.width, im_c),
            dtype=image.dtype)
        expand_image[:, :, :] = self.mean
        expand_image[:im_h, :im_w] = image

        if depth is not None:
            depth_h, depth_w, depth_c = depth.shape
            expand_depth = np.zeros(
                (self.height, self.width, depth_c),
                dtype=depth.dtype
            )
            expand_depth[:depth_h, :depth_w] = depth
        else:
            expand_depth = None
        ...
        if self.pad_gt:
            ...
            if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None): 
                ...
                expand_pos_masks = np.zeros(
                    (pos_masks.shape[0], self.height, self.width),
                    dtype=pos_masks.dtype)
                expand_pos_masks[:,:im_h,:im_w] = pos_masks
                pos_masks = expand_pos_masks

                expand_ang_masks = np.zeros(
                    (ang_masks.shape[0], self.height, self.width),
                    dtype=ang_masks.dtype)
                expand_ang_masks[:,:im_h,:im_w] = ang_masks
                ang_masks = expand_ang_masks

                expand_wid_masks = np.zeros(
                    (wid_masks.shape[0], self.height, self.width),
                    dtype=wid_masks.dtype)
                expand_wid_masks[:,:im_h,:im_w] = wid_masks
                wid_masks = expand_wid_masks

        return expand_image, masks, boxes, labels, expand_depth, pos_masks, ang_masks, wid_masks
...
class Resize(object):
    ...
    def __call__(self, image, masks, boxes, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        if self.resize_gt:
            ...
            if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
                ...
                pos_masks = pos_masks.transpose((1,2,0))
                pos_masks = cv2.resize(pos_masks, (width, height))
                ang_masks = ang_masks.transpose((1,2,0))
                ang_masks = cv2.resize(ang_masks, (width, height))
                wid_masks = wid_masks.transpose((1,2,0))
                wid_masks = cv2.resize(wid_masks, (width, height))

                assert masks.shape == pos_masks.shape == ang_masks.shape == wid_masks.shape
            
            # OpenCV resizes a (w,h,1) array to (s,s), so fix that
            if len(masks.shape) == 2:
                masks = np.expand_dims(masks, 0)
                if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
                    pos_masks = np.expand_dims(pos_masks, 0)
                    ang_masks = np.expand_dims(ang_masks, 0)
                    wid_masks = np.expand_dims(wid_masks, 0)
            else:
                masks = masks.transpose((2, 0, 1))
                if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
                    pos_masks = pos_masks.transpose((2,0,1))
                    ang_masks = ang_masks.transpose((2,0,1))
                    wid_masks = wid_masks.transpose((2,0,1))
        ...
        masks = masks[keep]
        if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
            pos_masks = pos_masks[keep]
            ang_masks = ang_masks[keep]
            wid_masks = wid_masks[keep]
        boxes = boxes[keep]
...
class Expand(object):
    ...
    def __call__(self, image, masks, boxes, labels, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        if random.randint(2):
            return image, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
        ...
        expand_image = np.zeros(
            (int(height*ratio), int(width*ratio), channel),
            dtype=image.dtype)
        expand_image[:, :, :] = self.mean
        ...
        if depth is not None:
            depth_h, depth_w, depth_c = depth.shape
            expand_depth = np.zeros(
                (int(height*ratio), int(width*ratio), depth_c),
                dtype=depth.dtype)
            expand_depth[int(top):int(top + height),
                        int(left):int(left + width)] = depth
            depth = expand_depth
        ...
        if (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
            expand_pos_masks = np.zeros(
                (pos_masks.shape[0], int(height*ratio), int(width*ratio)),
                dtype=pos_masks.dtype)
            expand_pos_masks[:,int(top):int(top + height),
                        int(left):int(left + width)] = pos_masks
            pos_masks = expand_pos_masks

            expand_ang_masks = np.zeros(
                (ang_masks.shape[0], int(height*ratio), int(width*ratio)),
                dtype=ang_masks.dtype)
            expand_pos_masks[:,int(top):int(top + height),
                        int(left):int(left + width)] = ang_masks
            ang_masks = expand_ang_masks

            expand_wid_masks = np.zeros(
                (wid_masks.shape[0], int(height*ratio), int(width*ratio)),
                dtype=wid_masks.dtype)
            expand_wid_masks[:,int(top):int(top + height),
                        int(left):int(left + width)] = wid_masks
            wid_masks = expand_wid_masks
...
class RandomMirror(object):
    def __call__(self, image, masks, boxes, labels, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        if random.randint(2):
            image = image[:, ::-1]
            masks = masks[:, :, ::-1]
            if (depth is not None) and (pos_masks is not None) and (ang_masks is not None) and (wid_masks is not None):
                depth = depth[:, ::-1]
                pos_masks = pos_masks[:, :, ::-1]
                ang_masks = ang_masks[:, :, ::-1]
                wid_masks = wid_masks[:, :, ::-1]
            boxes = boxes.copy()
...
class RandomRot90(object):
    def __call__(self, image, masks, boxes, labels, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        pos_masks = np.array([np.rot90(pos_mask, k) for pos_mask in pos_masks])
        ang_masks = np.array([np.rot90(ang_mask, k) for ang_mask in ang_masks])
        wid_masks = np.array([np.rot90(wid_mask, k) for wid_mask in wid_masks])
...
class PhotometricDistort(object):
    ...
    def __call__(self, image, masks, boxes, labels, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        im = image.copy()
        im, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks = self.rand_brightness(im, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks)
        if random.randint(2):
            distort = Compose(self.pd[:-1])
        else:
            distort = Compose(self.pd[1:])
        im, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks = distort(im, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks)
        return self.rand_light_noise(im, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks)
...
class PrepareMasks(object):
    ...
    def __call__(self, image, masks, boxes, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        return image, new_masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
class BackboneTransform(object):
    ...
    def __call__(self, img, masks=None, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
        ...
        return img.astype(np.float32), masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
def do_nothing(img=None, masks=None, boxes=None, labels=None, depth=None, pos_masks = None, ang_masks = None, wid_masks = None):
    return img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks
...
class GraspnetAugmentation(object):
    """ Transform to be used when training. """
    ...
    def __call__(self, img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks):
        return self.augment(img, masks, boxes, labels, depth, pos_masks, ang_masks, wid_masks)
