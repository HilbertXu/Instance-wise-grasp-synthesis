...
def remove_small_box(boxes, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, labels, area_limit):
    box_areas = (boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])
    keep = box_areas > area_limit
    return boxes[keep], masks[keep], gr_pos_masks[keep], gr_ang_masks[keep], gr_wid_masks[keep], labels[keep]
...
def crop(ori_h, crop_h, ori_w, crop_w, img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes, labels, keep_ratio=0.3):
    ...
    return img_cropped, masks_remained, gr_pos_masks_remained, gr_ang_masks_remained, gr_wid_masks_remained, boxes_remained[:, :4], boxes_remained[:, 4]
...
def pad_to_square(img, masks=None, gr_pos_masks=None, gr_ang_masks=None, gr_wid_masks=None, boxes=None, during_training=False):
    img_h, img_w = img.shape[:2]
    if img_h == img_w:
        return (img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes) if during_training else img
    else:
        ...
        if during_training:
            ...
            return pad_img, pad_masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes
        else:
            pad_img[0: img_h, 0: img_w, :] = img
            return pad_img


def multi_scale_resize(img, masks=None, gr_pos_masks=None, gr_ang_masks=None, gr_wid_masks=None, boxes=None, resize_range=None, during_training=False):
    ...
    if during_training:
        ...
        return img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes
    else:
        return cv2.resize(img, (resize_range, resize_range))


def to_train_size(img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes, labels, train_size):
    img_size = img.shape[0]

    if img_size == train_size:
        return img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes, labels
        ...
        boxes[:, [1, 3]] += random_y1
        boxes[:, [0, 2]] += random_x1
        return pad_img, pad_masks, pad_pos_masks, pad_ang_masks, pad_wid_masks, boxes, labels
    else:
        return crop(img_size, train_size, img_size, train_size, img, masks, gr_pos_masks, gr_ang_masks, gr_wid_masks, boxes, labels)
