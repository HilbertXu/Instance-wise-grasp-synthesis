...
class PredictionModule(nn.Module):
    def __init__(self, cfg, coef_dim=32):
        ...
        self.act_coef_layers = [nn.Sequential(nn.Conv2d(256, len(cfg.aspect_ratios) * self.coef_dim,
                                                  kernel_size=3, padding=1),
                                        nn.Tanh()).cuda() for i in range(11)]
    ...
    def forward(self, x):
        ...
        gr_act_coefs = []
        for layer in self.act_coef_layers:
            gr_act_coefs.append(layer(x).permute(0, 2, 3, 1).reshape(x.size(0), -1, self.coef_dim))

        return conf, box, gr_pos_coef, gr_sin_coef, gr_cos_coef, gr_wid_coef, gr_act_coefs
...
class GraspYolact(nn.Module):
    ...
    def forward(self, img, box_classes=None, pos_mask_gt=None, qua_mask_gt=None, sin_mask_gt=None, cos_mask_gt=None, wid_mask_gt=None, act_mask_gt=None, loss_masks=None):
        ...
        gr_act_coef_pred = [[] for i in range(11)]

        for aa in outs:
            class_p, box_p, gr_pos_coef_p, gr_sin_coef_p, gr_cos_coef_p, gr_wid_coef_p, gr_act_coef_p = self.prediction_layers(aa)
            ...
            for i in range(11):
                gr_act_coef_pred[i].append(gr_act_coef_p[i])


        class_pred = torch.cat(class_pred, dim=1)
        box_pred = torch.cat(box_pred, dim=1)

        gr_pos_coef_pred = torch.cat(gr_pos_coef_pred, dim=1) # [8, 18525, 32]
        gr_sin_coef_pred = torch.cat(gr_sin_coef_pred, dim=1) # [8, 18525, 32]
        gr_cos_coef_pred = torch.cat(gr_cos_coef_pred, dim=1) # [8, 18525, 32]
        gr_wid_coef_pred = torch.cat(gr_wid_coef_pred, dim=1) # [8, 18525, 32]

        for i in range(11):
            gr_act_coef_pred[i] = torch.cat(gr_act_coef_pred[i], dim=1).unsqueeze(2)

        gr_act_coef_pred = torch.cat(gr_act_coef_pred, dim=2)
        # [8, 18525, 11, 32]
        ...
        if self.training:
            ...
            return self.compute_loss(
                ...
                pos_mask_gt, qua_mask_gt, sin_mask_gt, cos_mask_gt, wid_mask_gt, act_mask_gt,
                loss_masks
            )
        else:
            class_pred = F.softmax(class_pred, -1)
            return class_pred, box_pred, gr_pos_coef_pred, gr_sin_coef_pred, gr_cos_coef_pred, gr_wid_coef_pred, gr_act_coef_pred, proto_out

    def compute_loss(
        self, 
        class_p, 
        box_p, 
        pos_coef_p, sin_coef_p, cos_coef_p, wid_coef_p, gr_act_coef_p,
        proto_p, box_class,
        ...
        loss_g_pos, loss_g_qua, loss_g_ang, loss_g_wid = self.lincomb_grasp_mask_loss(
        ...
        loss_g_act = self.lincomb_affordance_mask_loss(
            pos_bool, anchor_max_i, 
            gr_act_coef_p,
            proto_p,
            act_mask_gt,
    ...
    def lincomb_affordance_mask_loss(
        self, 
        pos_bool, anchor_max_i, 
        gr_act_coef_p, 
        proto_p, 
        ...
        for i in range(gr_act_coef_p.size(0)):
            loss_mask = loss_masks[i]
            ds_masks = F.interpolate(aff_mask_gt[i], (proto_h, proto_w), mode='bilinear',
                                              align_corners=False)
            ds_masks = ds_masks.permute(2, 3, 1, 0).contiguous().float()
            ds_masks = ds_masks.gt(0.5).float()


            act_anchor_i = anchor_max_i[i][pos_bool[i]]
            act_anchor_box = anchor_max_gt[i][pos_bool[i]]
            act_coef = gr_act_coef_p[i][pos_bool[i], :, :]

            if act_anchor_i.size(0) == 0:
                continue
            
            old_num_pos = act_coef.size(0)
            if old_num_pos > self.cfg.masks_to_train:
                perm = torch.randperm(act_coef.size(0))
                select = perm[:self.cfg.masks_to_train]
                act_coef = act_coef[select]
                act_anchor_i = act_anchor_i[select]
                act_anchor_box = act_anchor_box[select]
            
            num_pos = act_coef.size(0) # [num_pos, num_act, coef_dim]
            num_act = act_coef.shape[1]
            anchor_area = (act_anchor_box[:, 2] - act_anchor_box[:, 0]) * (act_anchor_box[:, 3] - act_anchor_box[:, 1])

            # print("4: ", act_coef.shape) 

            act_masks_gt = ds_masks[:, :, :, act_anchor_i]
            loss_mask = loss_mask[act_anchor_i, :]

            # print("5: ", act_masks_gt.shape)

            # print("6: ", proto_p[i].shape)

            act_masks_p_list = []

            for j in range(num_act):
                act_masks_p = torch.sigmoid(proto_p[i] @ act_coef[:, j, :].squeeze().t())
                act_masks_p = crop(act_masks_p, act_anchor_box)
                act_masks_p_list.append(act_masks_p.permute(2, 0, 1).unsqueeze(1))
            
            act_masks_p = torch.clamp(torch.cat(act_masks_p_list, dim=1), 0, 1)
            # print("7: ", act_masks_p.shape)

            # act_masks_loss = F.smooth_l1_loss(act_masks_p, act_masks_gt.permute(3, 2, 0, 1), reduction='none')
            act_masks_loss = F.binary_cross_entropy(act_masks_p, act_masks_gt.permute(3, 2, 0, 1), reduction='none')
            # act_masks_loss = act_masks_loss.mean(dim=1)
            
            act_masks_loss = act_masks_loss.sum(dim=(2,3)) * loss_mask
            act_masks_loss = act_masks_loss.sum(dim=1) / anchor_area / loss_mask.sum(dim=1)
            # act_masks_loss = act_masks_loss.sum(dim=(2,3))

            # print(act_masks_loss[0])
            # print((act_masks_loss * loss_mask)[0])

            # print("8: ", act_masks_loss.shape)


            if old_num_pos > num_pos:
                act_masks_loss *= old_num_pos / num_pos
            
            loss_aff += torch.sum(act_masks_loss) / num_pos
        
        return self.cfg.aff_alpha * loss_aff / proto_h / proto_w / total_pos_num
    ...
    def lincomb_grasp_mask_loss(
        ...
        return (
            ...
            self.cfg.grasp_alpha * 0.01 * loss_g_qua / proto_h / proto_w / total_pos_num,
